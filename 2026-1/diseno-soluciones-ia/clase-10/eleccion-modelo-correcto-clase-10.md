...
│ • Punto A: 3.2              │
│ • Punto B: 5.1              │
│ • Punto C: 1.8 ← MÁS CERCA  │
│ • Punto D: 6.4              │
│ • Punto E: 2.1              │
└────────┬────────────────────┘
         │
         v
PASO 3: ENCONTRAR K VECINOS MÁS CERCANOS
┌─────────────────────────────┐
│ Si K=3, selecciono:         │
│ • Punto C (1.8) ← Clase A   │
│ • Punto E (2.1) ← Clase A   │
...
└────────┬────────────────────┘
         │
         v
PASO 4: ASIGNAR CLASE MAYORITARIA
┌──────────────────────────────┐
│ Conteo de clases:            │
│ Clase A: 2 votos             │
│ Clase B: 1 voto              │
...
```
      ...
      B  │  ●Clase 2
      C  │    ●Clase 1
      L  │      ◆NUEVO PUNTO ← ¿Clase 1 o 2?
      A  │  ●Clase 2
      S  │     ●Clase 1
      E  └─────────────── Eje X
         DISTANCIA CORTA = CLASIFICACIÓN SIMILAR
         DISTANCIA LARGA = CLASIFICACIÓN DIFERENTE
```
...
(Datos)              (Procesamiento)     (Procesamiento)     (Predicción)
...
1. FORWARD PASS (Propagación hacia adelante)
   ...
   Cada neurona calcula: output = f(suma ponderada + sesgo)

2. CÁLCULO DEL ERROR
   Error = Predicción - Valor Real
   
3. BACKPROPAGATION (Retropropagación)
   El error viaja hacia atrás ajustando PESOS
...
| Tipo | Uso | Ejemplo |
|------|-----|---------|
| **ANN** (Artificial Neural Network) | Problemas generales | Predicción, clasificación |
| **CNN** (Convolutional) | Visión por computadora | Detección de objetos en imágenes |
| **RNN** (Recurrent) | Series temporales y secuencias | Reconocimiento de voz, traducción |
| **Transformer** | Procesamiento de lenguaje | ChatGPT, BERT, traducción |
...
Complejidad
    ↑
    │       ╔═════════════╗
    │       ║  Transformer║  ← Más complejo, más preciso
    │       ║   (ChatGPT) ║
    │       ╠═════════════╣
    │       ║   CNN/RNN   ║
    │       ╠═════════════╣
    │       ║  Red Dense  ║
    │       ╠═════════════╣
    │       ║  Árbol Dec. ║  ← Menos complejo, más simple
    │       ╚═════════════╝
    └──────────────────────→ Interpretabilidad
```
